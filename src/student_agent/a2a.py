"""In-process A2A protocol between the coordinator and the specialist agents.

Only the coordinator sends tasks; specialists can only reply to the task they received. Every
message carries the ``case_id`` it belongs to, the bus refuses cross-case replies, a repeated
``(recipient, intent)`` pair (loop) and anything beyond the per-case hop budget.
"""
...
MAX_HOPS = 12
# Longest task: payment-agent's three sequential MCP calls, each with one retry of 60 s.
TASK_TIMEOUT_S = 420.0
FAILED = "task_failed"


class A2AProtocolError(RuntimeError):
    """A message broke the protocol: wrong case, wrong peer, loop or exhausted hop budget."""
...
class A2ABus:
    """Routes coordinator tasks to specialists and traces each observable handoff."""
    ...
    async def request(
        ...
        if (recipient, intent) in self._seen:
        ...
        self._sent += 1
        self._seen.add((recipient, intent))
        task = A2AMessage(
        ...
        except Exception as exc:  # one broken specialist must not abort the whole case run
