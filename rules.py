"""Deterministic evidence rules shared by the specialist agents.

Every rule is anchored on the case's own ``get_order`` row. Rows returned by the other tools are
used only when their timestamps and amounts are consistent with that anchor; anything else is
reported as a data conflict and never used. Missing evidence stays missing: nothing here invents
a value, an identifier or an amount.
"""
...
# A capture belongs to the order when it happens within this window after approval.
CAPTURE_WINDOW = timedelta(hours=24)
# A reconciliation mismatch belongs to the order when it follows one of its captures this quickly.
MISMATCH_WINDOW = timedelta(hours=24)
# Fallback horizon for item shipping limits when the order has no estimated delivery date.
...
def parse_time(value: Any) -> datetime | None:
    """Parse an ISO-8601 timestamp; naive values are treated as UTC, invalid ones as missing."""
...
def parse_money(value: Any) -> Decimal | None:
    """Parse a non-negative BRL amount rounded to cents; invalid values are treated as missing."""
...
def _unique_rows(rows: Iterable[Mapping[str, Any]]) -> tuple[list[Mapping[str, Any]], int]:
    """Collapse identical rows (replication artifacts); return (rows, collapsed_count)."""
...
def order_facts(data: Any, order_id: str) -> OrderFacts | None:
    """Return the authoritative order row for ``order_id`` or ``None`` when it is absent."""
    ...
    return OrderFacts(
        order_id=order_id,
        status=(_text(row.get("order_status")) or "unknown").lower(),
...
def analyse_items(order: OrderFacts, data: Any) -> ItemView:
    """Keep item rows whose seller shipping limit falls inside the order's own delivery window."""
    raw = _rows(data)
    ...
    view = ItemView(collapsed_rows=collapsed)
    for row in unique:
        item_id = _text(row.get("order_item_id"))
        price = parse_money(row.get("price"))
        freight = parse_money(row.get("freight_value"))
        limit = parse_time(row.get("shipping_limit_date"))
        if (
            _text(row.get("order_id")) != order.order_id
            or item_id is None
            or price is None
            or freight is None
            or limit is None
...
def responsible_sellers(order: OrderFacts | None, items: ItemView, issue: str) -> list[str]:
    """In-scope sellers held responsible: late-handoff lines for seller delays, all lines else."""
...
def known_sellers(data: Any, seller_ids: Iterable[str]) -> list[str]:
    """Return the in-scope seller ids that the seller registry actually confirms."""
...
class PaymentView:
    ...
    def captured_total(self) -> Decimal:
        return sum((event.amount or ZERO for event in self.captures), ZERO)


def _events(data: Any) -> list[Mapping[str, Any]]:
    if isinstance(data, Mapping):
        return _rows(data.get("events"))
    return []


def _money_event(row: Mapping[str, Any]) -> MoneyEvent | None:
    at = parse_time(row.get("event_at"))
    kind = _text(row.get("event_type"))
    if at is None or kind is None:
        return None
    return MoneyEvent(
        at, kind.lower(), parse_money(row.get("amount_brl")), (_text(row.get("status")) or "")
    )
...
def _payment_row(row: Mapping[str, Any]) -> PaymentRow | None:
    ...
    if value is None:
        return None
    return PaymentRow(_text(row.get("payment_sequential")), _text(row.get("payment_type")), value)


def analyse_payments(
    order: OrderFacts, timeline: Any, payment_rows: Any, opened_at: datetime | None
) -> PaymentView:
    """Select captures and mismatches that belong to this order's payment approval window."""
    view = PaymentView()
    anchor = order.payment_anchor
    raw_events = [
        row for row in _events(timeline) if _text(row.get("order_id")) in (None, order.order_id)
    ]
    ...
    for event in sorted(parsed, key=lambda item: item.at):
        ...
        in_window = (
            ...
            and event.status.lower() == "confirmed"
        )
        if in_window:
    ...
    for event in sorted(parsed, key=lambda item: item.at):
        if event.kind == "captured":
            continue
        follows_capture = any(
            capture.at <= event.at <= capture.at + MISMATCH_WINDOW
            and (event.amount is None or money_equal(event.amount, capture.amount))
            for capture in view.captures
        )
        if event.kind == "reconciliation_mismatch" and follows_capture:
            view.mismatches.append(event)
        else:
            view.excluded_events += 1

    # Payment rows carry no timestamps: keep only rows that match an in-scope capture amount.
    rows_source = payment_rows if payment_rows is not None else _timeline_payments(timeline)
    scoped_rows = [
        row for row in _rows(rows_source) if _text(row.get("order_id")) in (None, order.order_id)
    ]
    ...
    for capture in view.captures:
        match = next((row for row in remaining if money_equal(row.value, capture.amount)), None)
        if match is None:
...
class RefundView:
    ...
    def with_status(self, status: str) -> list[MoneyEvent]:
        return [event for event in self.events if event.status.lower() == status]


def analyse_refunds(
    order: OrderFacts, data: Any, captures: list[MoneyEvent], opened_at: datetime | None
) -> RefundView:
    """Keep refund events raised after this order's payment and before the case was opened."""
    view = RefundView(available=data is not None)
    anchor = order.payment_anchor
    largest_capture = max((c.amount for c in captures if c.amount is not None), default=None)
    ...
    for event in sorted(filter(None, map(_money_event, unique)), key=lambda item: item.at):
        in_scope = (
            ...
            and anchor < event.at
            and (opened_at is None or event.at <= opened_at)
            and largest_capture is not None
            and event.amount is not None
            and event.amount <= largest_capture + CENT / 2
        )
        if in_scope:
...
def analyse_shipment(order: OrderFacts, items: ItemView, data: Any) -> ShipmentView:
    """Decide lateness from the authoritative order dates and corroborate with shipment data."""
    ...
    for row in _events(data):
        at = parse_time(row.get("event_at"))
        kind = (_text(row.get("event_type")) or "").lower()
        matches_delivery = (
            kind == "delivered_late"
        ...
        if matches_delivery and view.late_event_actor is None:
            view.late_event_actor = _text(row.get("actor"))
        else:
            view.excluded_events += 1
    return view
...
def _duplicate_capture(payments: PaymentView, order_value: Decimal) -> MoneyEvent | None:
    """Return the repeated capture when the same amount was captured twice beyond order value."""
    ...
    for amount, captures in by_amount.items():
        if len(captures) < 2 or len({c.at for c in captures}) < 2:
            continue
        sequences = {row.sequential for row in payments.matched_rows if row.value == amount}
        if len(sequences) >= 2 or not payments.matched_rows:
            return captures[1]
    return None
...
def diagnose(
    ...
    """Apply the issue priority: order state, delivery, refund state, then payment anomalies."""
    if order is None:
    ...
    if order.status in ("canceled", "unavailable") and payments.captures:
        issue = f"{order.status}_order_paid"
        return Diagnosis(
            issue, captured, order.order_id, (f"status_{order.status}", "capture_in_window"),
            tuple(doubts),
        )

    if shipment.delivered_late:
        seller_late = shipment.handoff_after_limit
        if seller_late is None and shipment.late_event_actor:
            seller_late = shipment.late_event_actor == "seller"
        if seller_late is None:
            doubts.append("late_actor_unknown")
        expected_actor = "seller" if seller_late else "logistics_provider"
        if shipment.late_event_actor and shipment.late_event_actor != expected_actor:
            doubts.append("late_event_actor_disagrees")
        issue = "late_delivery_seller" if seller_late else "late_delivery_logistics"
        basis = min(items.freight_total, captured) if payments.captures else items.freight_total
    ...
    failed = refunds.with_status("failed")
    if failed:
        basis = sum((event.amount or ZERO for event in failed), ZERO)
        return Diagnosis(
            "refund_failed", basis, order.order_id, ("refund_failed_in_window",), tuple(doubts)
        )
    if refunds.with_status("pending"):
        return Diagnosis(
            "refund_pending", ZERO, order.order_id, ("refund_pending_in_window",), tuple(doubts)
        )

    if payments.mismatches:
        basis = sum((event.amount or ZERO for event in payments.mismatches), ZERO)
        return Diagnosis(
            "payment_mismatch", basis, order.order_id, ("reconciliation_mismatch_open",),
            tuple(doubts),
        )
    ...
    if duplicate is not None and duplicate.amount is not None:
        return Diagnosis(
            "duplicate_charge", duplicate.amount, order.order_id,
            ("repeated_capture_distinct_sequence", "captured_above_order_value"), tuple(doubts),
        )
    if (
        ...
        return Diagnosis(
            "valid_split_payment", ZERO, order.order_id, ("captures_sum_to_order_value",),
            tuple(doubts),
        )
    if not payments.captures:
        doubts.append("no_capture_in_window")
    return Diagnosis(
        "unsupported_claim", ZERO, order.order_id, ("no_policy_trigger_in_evidence",),
        tuple(doubts),
    )
